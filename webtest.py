...
class FastClashPanel(wx.Panel):
    def __init__(self, parent):
        ...
        self.url_text_ctrl = wx.TextCtrl(self, style=wx.TE_MULTILINE, size=(300, 30))

        vbox.Add(wx.StaticText(self, label="程序:"), 0, wx.ALL, 5)
        vbox.Add(self.file_picker1, 0, wx.EXPAND | wx.ALL, 5)
        vbox.Add(wx.StaticText(self, label="配置文件夹:"), 0, wx.ALL, 5)
        vbox.Add(self.dir_picker, 0, wx.EXPAND | wx.ALL, 5)
        vbox.Add(wx.StaticText(self, label="输入网址:"), 0, wx.ALL, 5)
        
        # URL input + download button
        hbox_url = wx.BoxSizer(wx.HORIZONTAL)
        hbox_url.Add(self.url_text_ctrl, 1, wx.EXPAND | wx.ALL, 5)
        download_btn = wx.Button(self, label="下载文件")
        download_btn.SetMinSize((-1, 50))
        hbox_url.Add(download_btn, 0, wx.ALL, 5)
        vbox.Add(hbox_url, 0, wx.EXPAND)
        ...
        # Program status
        self.status_text = wx.StaticText(self, label="程序状态: 停止运行", style=wx.ALIGN_CENTER)
    ...
    def on_path_change(self, event):
        ...
        try:
            with open(CONFIG_FILE, "r", encoding="utf-8") as f:
                ...
                secret = lines[3] if len(lines) > 3 else ""
                testurl = lines[4] if len(lines) > 4 else ""
...
class WebTestPanel(wx.Panel):
    def __init__(self, parent):
        ...
        # New: Clash Secret text control
        self.clash_secret_text_ctrl = wx.TextCtrl(self)
        self.clash_secret_text_ctrl.Bind(wx.EVT_TEXT, self.on_path_change)
        vbox.Add(wx.StaticText(self, label="面板密码:"), 0, wx.ALL, 5)
        vbox.Add(self.clash_secret_text_ctrl, 0, wx.EXPAND | wx.ALL, 5)

        # New: Clash testurl text control
        self.clash_testurl_text_ctrl = wx.TextCtrl(self)
        self.clash_testurl_text_ctrl.Bind(wx.EVT_TEXT, self.on_path_change)
        vbox.Add(wx.StaticText(self, label="测试url:"), 0, wx.ALL, 5)
        vbox.Add(self.clash_testurl_text_ctrl, 0, wx.EXPAND | wx.ALL, 5)
        ...
        if os.path.exists(CONFIG_FILE):
            try:
                with open(CONFIG_FILE, "r", encoding="utf-8") as f:
                    lines = f.read().splitlines()
                    if len(lines) > 3:
                        self.clash_secret_text_ctrl.SetValue(lines[3])
                    if len(lines) > 4:
                        self.clash_testurl_text_ctrl.SetValue(lines[4])
            except (IOError, IndexError):
                pass
